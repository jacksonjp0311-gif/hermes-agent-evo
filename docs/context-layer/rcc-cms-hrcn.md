...
```text
Hermes = actor / runtime / body
RCC    = repository orientation map
CMS    = governed memory, repair, dry-run, apply-gate, rollback, evidence, and permission layer
HRCN   = bridge contract between Hermes, RCC, CMS, and human authorization
...
README files inside folders are navigation surfaces, not runtime execution surfaces.
...
README.md
docs/context-layer/
*/README.md mini README files for existing top-level directories
```
...
- `docs/context-layer/cybernetic-lessons-injection.md`
- `docs/context-layer/hrcn-roadmap.md`
- `docs/context-layer/rcc-cms-hrcn.lesson-injection.validation.json`

Injected lesson spine:

```text
Evidence-level lessons must become public surface memory.
Final verifier residue must be committed or discarded.
Agent proposal is not authority.
Read-only context is not runtime integration.
Dry-run is not write authority.
Apply gate, apply packet, and dry-apply sandbox are not live authority.
Validation order is part of system geometry.
Profile adoption is not validation.
```

RCC-N v1.7 profile lock:

```text
Select the lightest sufficient RCC-N/HRCN profile.
Do not over-govern simple repositories.
Do not under-govern high-risk repositories.
Do not claim utility without baseline comparison and cost accounting.
```
