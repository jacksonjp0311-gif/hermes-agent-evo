...
def test_alignment_guard_preflight_accepts_current_repo_shape():
    ...
    assert result.checks["root_readme_rhp006_1_passed"] is True
    assert result.checks["root_public_metrics_current_ops"] is True
    assert result.checks["root_public_metrics_latest_rhp"] is True
    assert result.checks["post_seal_chart_rhp006_1"] is True
...
def test_alignment_guard_final_requires_green_latest_evidence():
    ...
    assert result.checks["latest_rhp006_1_passed"] is True
