...
def test_hrcn_bridge_status_exposes_v03_anchor():
    ...
    assert status.authority["memory_promotion"] is False
...
def test_hrcn_context_packet_is_interface_safe_v03():
    packet = bridge.make_gui_context_packet(Path.cwd())
    assert packet["packet_schema"] == "HRCN-GUI-RUNTIME-CONTEXT-PACKET-v0.3"
    assert "mutate runtime source" in packet["forbidden_runtime_use"]
    assert "write memory" in packet["forbidden_runtime_use"]
    assert "promote memory" in packet["forbidden_runtime_use"]
